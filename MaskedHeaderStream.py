...
def unObfuscate(
    jDict: dict, offset: int = 0, streamPos: int = 0, headerLength: int = 256
):
    """
    Args:
    ...
    md5NameDict = {it["md5"]: it["name"] for it in assetList}
    md5TypeDict = {it["md5"]: it["type"] for it in assetList}
    ...
    for path in filePaths:
        ...
        if buff[0:5] == unitySignature:
            ...
            exportFolder = (
                Path(__outputPath).joinpath(str(jDict["revision"])).joinpath(_type)
            )
            ...
            unityFS = __cryptByString(
                buff, md5NameDict[md5], offset, streamPos, headerLength
            )
...
def rename(jDict: dict):
    ...
    md5NameDict = {it["md5"]: it["name"] for it in resourceList}
    md5TypeDict = {it["md5"]: it["type"] for it in resourceList}
    contents = Path(__inputDirectory).glob("**/*")
    try:
        filePaths = [path for path in contents if path.name in md5NameDict]
    except FileNotFoundError:
...
def __stringToMaskBytes(
    maskString: str, maskStringLength: int, bytesLength: int
) -> bytes:
    ...
    if maskString != 0:
        if maskStringLength >= 1:
            ...
            while maskStringLength != j:
                ...
                maskBytes[k] = (
                    charJ  # .to_bytes(length=1, byteorder='little', signed=True)
                )
                k -= 2
        if bytesLength >= 1:
            l = bytesLength
...
def __cryptByString(
    input: bytes, maskString: str, offset: int, streamPos: int, headerLength: int
) -> bytes:
