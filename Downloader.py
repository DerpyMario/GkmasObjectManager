...
def download(
    jdic: dict, download_dir: str, download_asset: int, download_resource: int
):
    ...
    if download_asset:
        ...
        for asset in assetBundleList:
            ...
            if os.path.exists(file_path):
                print(
                    f">>> [Skipped] ({count}/{assetnum}) Assetbundle '{ab_name}.unity3d' saved as {md5_name} has existed."
                )
            else:
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f">>> [Succeed] ({count}/{assetnum}) Assetbundle '{ab_name}.unity3d' has been saved as {md5_name}."
                    )
                else:
                    print(
                        f">>> [Failed] ({count}/{assetnum}) Assetbundle '{ab_name}.unity3d' saved as {md5_name} failed."
                    )
            count = count + 1
    ...
    if download_resource:
        ...
        for res in resourceList:
            ...
            if os.path.exists(file_path):
                print(
                    f">>> [Skipped] ({count}/{resourcenum}) ResourceBundle '{res_name}' has existed."
                )
            else:
                response = requests.get(url)
                if response.status_code == 200:
                    with open(file_path, "wb") as f:
                        f.write(response.content)
                    print(
                        f">>> [Succeed] ({count}/{resourcenum}) ResourceBundle '{res_name}' has been saved."
                    )
                else:
                    print(
                        f">>> [Failed] ({count}/{resourcenum}) ResourceBundle '{res_name}' saving failed."
                    )
            count = count + 1
